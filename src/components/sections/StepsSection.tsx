...
export function StepsSection({ data, stepsImage, stepImages, imagesLoading }: StepsSectionProps) {
  ...
  const handleVerMais = async (passoNumero: number, exemplo: string) => {
    ...
    try {
      const { data: fnData, error: fnError } = await supabase.functions.invoke('explain-example', {
        body: { exemplo, contexto: data.titulo || "" },
      });

      if (fnError) throw new Error(fnError.message || "Erro na função.");
      if (!fnData?.explicacao) throw new Error("Resposta vazia da IA.");

      setExplanations(prev => ({ ...prev, [passoNumero]: fnData.explicacao }));
    } catch (error: any) {
